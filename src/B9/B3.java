...
//                        continue;
    ...
    public void solve() { 
        try {
            ...
            int[] restT = copyOf(restS, length);
            
            int unMatchCnt = 0;
            
            for (int i = 0; i < N; i++) {
                for(int j = 0; j < length; j++) {
                    if (restT[j] == 0) continue;

                    int roopUnMatchCnt = unMatchCnt;
                    int restSIndex = cA[i] - 'a';
                    if (j != restSIndex) roopUnMatchCnt++;
                    restT[j]--;
                    restS[restSIndex]--;
                    ...
                    for(int k = 0; k < length; k++) {
                        restCnt -= Math.min(restS[k], restT[k]);
                    }
                    if (restCnt + roopUnMatchCnt > K) {
                        restT[j]++;
                        restS[restSIndex]++;
                        continue;
                    }
